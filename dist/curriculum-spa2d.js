...
class CurriculumAPI {
    ...
    async completeActivity(activityId, bookName) {
        ...
        // Format date for Knack (MM/DD/YYYY hh:mm am/pm)
        const now = new Date();
        const knackDate = `${now.getMonth()+1}/${now.getDate()}/${now.getFullYear()} ${now.getHours()%12||12}:${String(now.getMinutes()).padStart(2,'0')} ${now.getHours()>=12?'pm':'am'}`;
        
        const data = {
            field_1437: [user.id],
            field_1432: JSON.stringify(json),
            field_1449: user.name, // Tutor name
            field_2295: knackDate // Knack date format
        };
