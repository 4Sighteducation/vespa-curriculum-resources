...
class CurriculumAPI {
    ...
    async completeActivity(activityId, bookName) {
        ...
        // Knack will auto-populate date field, just send the required fields
        const data = {
            field_1437: [user.id], // Connection to tutor
            field_1432: JSON.stringify(json) // JSON of completed activities
            // Don't send field_1449 or field_2295 - Knack will handle them
        };
...
/**
 * Pages 1, 2, 3 with fixes for search, sorting, and problem matching
 */
...
const P2 = {
    ...
    render() {
        ...
        c.innerHTML = `
            <div class="browser-top">
                ...
                <div class="filters-row">
                    <input type="text" id="search-input" class="search-input" placeholder="🔍 Search by name, theme, or ID..." 
        ...
        // FIXED: Add event listeners AFTER render (not oninput in HTML)
        setTimeout(() => {
            const searchInput = document.getElementById('search-input');
            if (searchInput) {
                searchInput.addEventListener('input', (e) => {
                    State.filters.search = e.target.value;
                    this.updateActivitiesList(); // Only update list, not whole page
                });
            }
            ...
            if (problemSelect) {
                problemSelect.addEventListener('change', (e) => {
                    this.searchByProblem(e.target.value);
                });
            }
        }, 0);
    ...
    // FIXED: Only update activities list, not search box
    ...
    searchByProblem(value) {
        ...
        const problems = {
            Vision: [
                ["21st Birthday", "Roadmap", "Personal Compass", "Perfect Day"],
                ["Motivation Diamond", "Five Roads", "Mission", "Medal"],
                ["Success", "Clues", "There", "Back", "20 Questions", "SMART Goals"]
            ],
            Effort: [
                ["Weekly Planner", "25min", "Sprints", "Priority Matrix", "Now vs Most"],
                ["Will vs Skill", "Thermometer", "Rocks", "Kill", "Critic"],
                ["Failing", "Growth Mindset", "2 Slow", "Mistakes"]
            ],
            Systems: [
                ["Weekly Planner", "Priority", "STQR", "Graphic", "Organisers"],
                ["Leitner", "Revision", "Questionnaire", "Spaced", "2-4-8"],
                ["Weekly", "Rule of 3", "Progress Chart", "Packing"]
            ],
            Practice: [
                ["Test Yourself", "Spaced", "Leitner", "2-4-8"],
                ["Time to Teach", "9 Box", "Practice", "Questionnaire"],
                ["Know the Skills", "Mechanical", "Flexible", "Right-Wrong"]
            ],
            Attitude: [
                ["Growth Mindset", "Battery", "Managing", "Reactions", "Stopping", "Negative"],
                ["Failing", "Benefit", "Change Curve", "First Aid"],
                ["Battery", "Network", "Vampire", "One to Ten"]
            ]
        };
        
        const searchTerms = problems[theme]?.[parseInt(idx)];
        if (searchTerms) {
            console.log('[P2] Problem search - looking for:', searchTerms);
            
            // FILTER using fuzzy matching
            const matched = State.activities.filter(a => U.fuzzyMatch(a.name, searchTerms));
            
            console.log('[P2] Matched', matched.length, 'activities');
            
            if (matched.length > 0) {
                // Show matched activities
                this.renderActivities(matched);
                U.toast(`Found ${matched.length} matching activities`);
                
                // Update search box to show what was found
                document.getElementById('search-input').value = `Found: ${matched.map(a => a.name).slice(0, 3).join(', ')}${matched.length > 3 ? '...' : ''}`;
            } else {
                U.toast('No matching activities found');
            }
        }
    },
    ...
    getProblemOptions() {
        const problems = {
            ...
            Effort: ['Struggles with homework', 'Gives up when difficult', 'Gets distracted easily'],
...
window.initializeCurriculumSPA = async function() {
    console.log('[SPA v2c] Initializing...');
    ...
    function init() {
        ...
        console.log('[SPA v2c] ✅ Ready! Fixed search, sorting, and problem matching!');
    }
};
...
console.log('[Curriculum SPA v2c] All loaded!');
