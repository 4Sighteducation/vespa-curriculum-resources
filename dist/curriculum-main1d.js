...
class CurriculumAPI {
    ...
    async fetchFromKnack(objectKey, filters = null, sort = null) {
        ...
        console.log('[Curriculum API] DEBUG - Fetch starting for:', objectKey);
        console.log('[Curriculum API] DEBUG - Config:', {
            appId: this.config.knackAppId,
            hasApiKey: !!this.config.knackApiKey,
            apiKeyLength: this.config.knackApiKey?.length
        });
        ...
        console.log('[Curriculum API] DEBUG - Fetching from:', fullUrl);
        console.log('[Curriculum API] DEBUG - Headers:', headers);

        try {
            const response = await fetch(fullUrl, { headers });
            console.log('[Curriculum API] DEBUG - Response status:', response.status);
            console.log('[Curriculum API] DEBUG - Response OK?:', response.ok);
            
            if (!response.ok) {
                const errorBody = await response.text();
                console.error('[Curriculum API] DEBUG - Error response:', errorBody);
                throw new Error(`Knack API error: ${response.status}`);
            }
            
            const data = await response.json();
            console.log('[Curriculum API] DEBUG - Response data:', data);
            console.log('[Curriculum API] DEBUG - Records count:', data.records?.length || 0);
            
            return data.records || [];
        } catch (error) {
            console.error('[Curriculum API] Fetch error:', error);
            console.error('[Curriculum API] Stack:', error.stack);
            throw error;
        }
    }
...
(function() {
    ...
    // FIXED: Get config fresh each time, not captured at script load
    function getConfig() {
        return window.CURRICULUM_RESOURCES_CONFIG || {};
    }
    
    function getSceneId() {
        return getConfig().sceneKey || 'scene_1280';
    }
    
    function getViewId() {
        return getConfig().viewKey || 'view_3244';
    }
    ...
    function initAPI() {
        ...
        // FIXED: Get fresh config each time
        const CONFIG = getConfig();
        console.log('[Page 1] DEBUG - Initializing API with config:', {
            hasAppId: !!CONFIG.knackAppId,
            hasApiKey: !!CONFIG.knackApiKey,
            appId: CONFIG.knackAppId,
            apiKeyLength: CONFIG.knackApiKey?.length
        });
    ...
    $(document).on(`knack-scene-render.${getSceneId().replace('scene_', '')}`, function() {
        log('Scene render event triggered');
        init();
    });
    
    $(document).on(`knack-view-render.${getViewId().replace('view_', '')}`, function() {
    ...
    if (window.location.hash.includes('tutor-activities') || 
        window.location.hash.includes(getSceneId().replace('scene_', ''))) {
